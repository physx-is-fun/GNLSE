...
def plotFirstAndLastPulse(Pulse,simulation:SIMULATION_config):
    ...
    initial_pulse = getPower(initial_pulse[simulation.nx // 2, simulation.ny // 2 , :])
    initial_pulse_maximum = np.max(initial_pulse)
    initial_pulse /= initial_pulse_maximum # Normalize
    final_pulse = Pulse[-1]
    final_pulse = getPower(final_pulse[simulation.nx // 2, simulation.ny // 2, :])
...
def plotFirstAndLastSpectrum(Pulse,simulation:SIMULATION_config):
    ...
    initial_spectrum = getPower(fftshift(fft(initial_pulse)))
    ...
    final_spectrum = getPower(fftshift(fft(final_pulse)))
...
def plotPeakPower(Pulse,simulation:SIMULATION_config):
    ...
    plt.title('Peak power evolution at center')
    savePlot('Peak power evolution at center')
    plt.tight_layout()
    plt.show()
...
def plotPulseEvolution(Pulse,simulation:SIMULATION_config):
    ...
    zt_matrix = np.array([getPower(A[simulation.nx // 2, simulation.ny // 2, :]) for A in Pulse])  # shape: (nz, nt)
...
def plotSpectrumEvolution(Pulse,simulation:SIMULATION_config):
    ...
    spectrum_zw = [
        getPower(fftshift(fft(A[simulation.nx // 2, simulation.ny // 2, :])))
        for A in Pulse
    ]
